...
export function getRentalHomeContents(formDetails, images, mode = "post") {
  ...
  const locationText = typeof safeLocation === 'string' && safeLocation.includes(",")
    ...
    : `${formDetails?.location_city || ""}, ${formDetails?.location_state || ""}, ${formDetails?.location_zipcode || ""}`;

  return [
    { text: "Property Type", value: formDetails?.property_type || "N/A" },
    { text: "Available From", value: (formDetails?.available_from || formDetails?.fromDate) ? dayjs(formDetails.available_from || formDetails.fromDate).format("DD-MM-YYYY") : "N/A" },
    { text: "Area", value: `${formDetails?.area || 0} m²` },
