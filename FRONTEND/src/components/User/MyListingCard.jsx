...
const MyListingCard = ({ item, category, onEdit, onDelete, onToggleStatus }) => {
    ...
    const title = item.first_name 
        ? (item.credential ? `Dr. ${item.first_name} ${item.last_name}` : `${item.first_name} ${item.last_name}`)
        : (item.title || item.course_title || item.event_name || item.type || item.make || item.home_type || "Listing");

    const subTitle = item.office_address_city 
        ? `${item.office_address_city}, ${item.office_address_state || ''}`
        : (item.primary_address_city 
            ? `${item.primary_address_city}, ${item.primary_address_state || ''}`
            : (item.location || item.address || `${item.location_city || ''} ${item.location_state || ''}`.trim() || "Local Listing"));

    const price = item.price || item.rent || item.deposit_rent || item.course_fee || item.ticket_price || (item.consultation_fee_amount ? `$${item.consultation_fee_amount}` : null);
    const image = item.profile_photo_url || item.poster_urls?.[0] || item.images?.[0] || item.photos?.[0] || (item.image_1) || (Array.isArray(item.cover_images) ? item.cover_images[0] : null) || null;
